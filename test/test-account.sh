...
[[ "$(jq '.message' $out)" != '"account, get"' ]] && echo " <<< error" && \
    false
echo; echo
...
curl -s -H "Authorization: Bearer invalid-token" \
    $apilink/api/account/123 | tee $out
[[ "$(jq '.message' $out)" != '"unauthorized"' ]] && echo " <<< error" && \
    false
echo; echo
...
curl -sX POST -H "Authorization: Bearer $ADMIN_TOKEN" \
    ...
    $apilink/api/account/ | tee $out
[[ "$(jq '.message' $out)" != '"account, post"' ]] && echo " <<< error" && \
    false
echo; echo
...
curl -sX DELETE -H "Authorization: Bearer $TOKEN" \
    $apilink/api/account/123 | tee $out
[[ "$(jq '.message' $out)" != '"account, delete"' ]] && echo " <<< error" && \
    false
echo; echo
...
[[ "$(jq '.message' $out)" != '"account, put"' ]] && echo " <<< error" && \
    false
echo; echo
...
curl -sX PATCH -H "Authorization: Bearer $TOKEN" \
    ...
    $apilink/api/account/id | tee $out
[[ "$(jq '.message' $out)" != '"account, patch"' ]] && echo " <<< error" && \
    false
echo; echo
...
curl -sX UNKNOWN -H "Authorization: Bearer $TOKEN" \
    $apilink/api/account/id | tee $out
[[ "$(jq '.message' $out)" != '"method not allowed"' ]] && echo " <<< error" &&
    \
    false
echo; echo
